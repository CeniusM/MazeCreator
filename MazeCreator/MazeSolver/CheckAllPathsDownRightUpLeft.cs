...
// point of this is exacly the same as CheckAllPathsDownRightUpLeft, ecxept for the fact it checks
// down then right then up then left
internal class CheckAllPathsDownRightUpLeft
{
    public Maze maze;
    ...
    public CheckAllPathsDownRightUpLeft(Maze maze)
    {
        this.maze = maze;
    ...
    public void Step(int stepSize = -1)
    {
        ...
        for (int i = 0; i < stepSize; i++)
        {
            ...
            while (true)
            {
                ...
                if (walker.coordsTestetCount == 0)
                    offSet = new Coord(0, -1);
                if (walker.coordsTestetCount == 1)
                    offSet = new Coord(1, 0);
                if (walker.coordsTestetCount == 2)
                    offSet = new Coord(0, 1);
                if (walker.coordsTestetCount == 3)
                    offSet = new Coord(-1, 0);
